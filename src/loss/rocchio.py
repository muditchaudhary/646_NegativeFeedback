...
def RochhiosLoss(args, refined_query_repr, query_repr, relevant_repr, negative_repr):
    ...
    regularized_loss = args.alpha1 * cosineLoss(refined_query_repr, query_repr, torch.tensor( 1.0) ) # want to keep them similar
    ...
    similar_repr = args.alpha2 * cosineLoss(unsq_refine_query.repeat(1, batch_pos, 1), relevant_repr, torch.tensor( 1.0) )
    dissimilar_repr = args.alpha3 * cosineLoss(unsq_refine_query.repeat(1, batch_neg, 1), negative_repr, torch.tensor(-1.0))
